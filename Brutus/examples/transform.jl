...
# This example isn't concerned with generating the actual payload IR so we just parse it:
payload = """
...
"""
mod = parse(IR.Module, payload)

# add the schedule to the module containing the payload:
push!(IR.body(mod), IR.rmfromparent!(IR.Operation(named_sequence_mod)))

# apply the schedule and get rid of the schedule in the module:
